...
public class pathtest										// check  shortest path  dfs
{
  ...
  static int i,j;
	 private static void dfs_rec(ArrayList<ArrayList<Integer>> adjLists, boolean[] visited, int v){
	        ...
	        for(int w : adjLists.get(v)){
	            if(!visited[w])
	            {
	            	...
	            	{
	            		 System.out.print(w + " ");
	            		System.out.println("given path is the  one");
	            		System.exit(0);
	            	}
	 ...
	 public static void dfs(ArrayList<ArrayList<Integer>> adjLists, int s){
	        ...
	        System.out.println("enter a source  node and dest. node");
	         i=sc.nextInt();
	         j=sc.nextInt();
	        dfs_rec(adjLists, visited, i);
	    }
